...
const check = function (): Report[] {
  const report: Report[] = [];
  let currentReport: Report;

  // eslint-disable-next-line no-process-env
  let nodeModulesFolderPath: string | undefined = process.env.p;

  if (!isValidNodeModulesFolderPath(nodeModulesFolderPath)) {
    nodeModulesFolderPath = getNodeModulesFolder(true);
  }

  if (typeof nodeModulesFolderPath !== 'string') {
    currentReport = {
      status: 'error',
  ...
  if (!nodeModulesFolderPath.endsWith('\\')) {
  ...
  const nodeModulesFolders = getNodeModules(nodeModulesFolderPath);
